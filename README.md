...
> **Precisão monetária:** todos os valores em dinheiro (despesas, saldo, salário) são tratados com `decimal.Decimal` em Python e persistidos em colunas `DECIMAL` no SQLite, evitando os erros de arredondamento típicos de `float` (ex.: `0.1 + 0.2 != 0.3`). Bancos criados por versões anteriores (colunas `REAL`) são migrados automaticamente na primeira abertura, sem perda de dados.
...
| Recurso | Descrição |
|---------|-----------|
| **Cadastro de Despesas** | Adicione despesas individuais com categoria, subcategoria (opcional), descrição e valor |
| **Preco unitario x Quantidade** | Informe preco unitario + quantidade (ex.: "Leite 1L" a R$ 4,50, comprando 2) e o total e calculado automaticamente |
| **Cadastro em Massa** | Cole múltiplas linhas no formato `Descrição, Valor` e importe de uma vez |
| **Árvore de Gastos** | Navegue hierarquicamente: **Ano/Mês → Categoria → Subcategoria (se usada) → Despesas** |
...
- **Casos de borda** (unicode, caracteres especiais, precisão decimal, migração de bancos antigos)
