...
CG.monthly = (function () {
    ...
    async function renderList() {
        ...
        try {
            ...
            if (!res.success || !res.data.length) {
                container.innerHTML = `
                    <div class="col-span-full text-center py-16 text-slate-400 space-y-3">
                        ...
                        <p>Nenhuma despesa mensal cadastrada.</p>
                        <p class="text-sm">Crie templates de gastos fixos (ex.: "Contas da casa", "Assinaturas") para serem lançados e debitados do saldo automaticamente todo mês.</p>
                    </div>`;
                return;
            }
                ...
                card.innerHTML = `
                    <div class="px-5 py-4 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between gap-3">
                        <div class="min-w-0">
                            ...
                            <p class="text-xs text-slate-400">${group.items.length} item(ns)</p>
                        </div>
                        <div class="flex items-center gap-2 shrink-0">
                            ${isApplied
                                ? '<span class="text-xs bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-300 px-2 py-1 rounded-full font-medium"><i class="ph ph-check"></i> Aplicado em ' + formatMonth(group.last_applied_month) + '</span>'
                                : '<span class="text-xs bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-300 px-2 py-1 rounded-full font-medium"><i class="ph ph-clock"></i> Pendente</span>'}
                        </div>
                    </div>
                    ...
                    <div class="px-5 py-3 bg-slate-50 dark:bg-slate-800/40 border-t border-slate-200 dark:border-slate-700 flex items-center justify-between">
                        <span class="text-sm text-slate-500 dark:text-slate-400">Total mensal</span>
                    ...
                    <div class="px-5 py-3 border-t border-slate-200 dark:border-slate-700 flex gap-2">
                        ...
                        <button onclick="CG.monthly.applyGroup(${group.id})"
                            class="px-3 py-1.5 rounded-lg text-sm bg-primary-600 hover:bg-primary-700 text-white transition flex items-center gap-1" title="Lançar as despesas deste mês agora e debitar do saldo">
                            <i class="ph ph-lightning"></i> Aplicar agora
                        </button>
            ...
            container.innerHTML = `<div class="col-span-full text-center py-12 text-red-400">Erro ao carregar despesas mensais.</div>`;
        }
    }
    ...
    function openForm(groupId = null) {
        ...
        document.getElementById('monthly-form-title').textContent = groupId ? 'Editar Despesa Mensal' : 'Nova Despesa Mensal';
        addItemRow();
        ...
        row.innerHTML = `
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <input type="text" class="mi-category px-3 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 focus:ring-2 focus:ring-primary-500 outline-none transition text-sm" list="category-list"
                    placeholder="Categoria (ex.: Moradia)" oninput="CG.monthly.updateFormTotal()">
                <input type="text" class="mi-subcategory px-3 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 focus:ring-2 focus:ring-primary-500 outline-none transition text-sm" list="subcategory-list"
                    placeholder="Subcategoria (opcional)" oninput="CG.monthly.updateFormTotal()">
            </div>
            <div class="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center">
                <input type="text" class="mi-name flex-1 px-3 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 focus:ring-2 focus:ring-primary-500 outline-none transition text-sm"
                    placeholder="Nome do produto / conta" oninput="CG.monthly.updateFormTotal()">
                <input type="number" class="mi-price w-full sm:w-28 px-3 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 focus:ring-2 focus:ring-primary-500 outline-none transition text-sm"
                    placeholder="Preço" step="0.01" min="0" oninput="CG.monthly.updateFormTotal()">
                ...
                <span class="mi-subtotal text-sm font-semibold text-slate-600 dark:text-slate-300 w-24 text-right shrink-0">R$ 0,00</span>
                <button type="button" onclick="CG.monthly.removeItemRow('${rowId}')"
                    class="text-red-500 hover:text-red-700 dark:hover:text-red-400 p-1 shrink-0" title="Remover item">
    ...
    async function submitForm() {
        ...
        if (!name) {
            CG.toast.show('Informe um nome para a despesa mensal.', 'warning');
            return;
        }
        ...
        if (hasInvalidRow) {
            CG.toast.show('Verifique os itens: categoria, nome, preço e quantidade são obrigatórios.', 'warning');
            return;
        }
        if (items.length === 0) {
            CG.toast.show('Adicione ao menos um item.', 'warning');
            return;
        }
        ...
        try {
            ...
            CG.toast.show('Erro ao salvar despesa mensal.', 'error');
        }
    }
    ...
    async function applyGroup(groupId) {
        try {
            ...
            CG.toast.show('Erro ao aplicar despesa mensal.', 'error');
        }
    }
